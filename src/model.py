...
class ModelConfig:
    ...
    pe_type: str = "learned"    # "learned" (standard/low-rank), "parametric_sin"
    pe_init_period: float = 11.0  # init period for parametric_sin
    # Token embedding rank (0 = full rank)
    token_emb_rank: int = 0
...
class ParametricSinPE(nn.Module):
    """Parametric sinusoidal position encoding.

    PE[pos, d] = amp[d] * sin(pos * freq[d] + phase[d])
    Params: 3 * d_model (vs max_seq_len * rank + rank * d_model for low-rank).
    """

    def __init__(self, max_len: int, d_model: int, init_period: float = 11.0):
        super().__init__()
        self.freq = nn.Parameter(torch.full((d_model,), 2 * math.pi / init_period))
        self.phase = nn.Parameter(torch.zeros(d_model))
        self.amp = nn.Parameter(torch.ones(d_model))

    def forward(self, positions: torch.Tensor) -> torch.Tensor:
        pos = positions.float().unsqueeze(-1)  # [..., 1]
        return self.amp * torch.sin(pos * self.freq + self.phase)
...
class TinyDecoderLM(nn.Module):
    def __init__(self, cfg: ModelConfig):
        ...
        # Token embedding
        if cfg.token_emb_rank > 0:
            self.token_emb = LowRankEmbedding(cfg.vocab_size, cfg.d_model, cfg.token_emb_rank)
        else:
            self.token_emb = nn.Embedding(cfg.vocab_size, cfg.d_model)

        # Position embedding
        if cfg.pe_type == "parametric_sin":
            self.pos_emb = ParametricSinPE(cfg.max_seq_len, cfg.d_model, cfg.pe_init_period)
        elif cfg.pos_rank > 0:
        ...
        if cfg.token_emb_rank > 0:
            # Low-rank token embedding: tied head computed in forward()
            pass
        else:
    ...
    def forward(
        ...
        x = self.ln_f(x)
        if self.cfg.token_emb_rank > 0:
            # Tied low-rank head: logits = (x @ B^T) @ A^T
            logits = (x @ self.token_emb.B.T) @ self.token_emb.A.T
        else:
